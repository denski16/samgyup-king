        ...
        <div className="flex min-h-screen bg-gray-50 text-gray-900 font-sans">
            ...
            <main className="flex-1 p-4 pt-20 md:p-8 overflow-y-auto w-full max-w-[100vw] overflow-x-hidden">
                <header className="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <button onClick={fetchBranchInventory} className="p-3 bg-white border border-gray-200 rounded-2xl hover:text-orange-500 shadow-sm transition-all active:scale-90">
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                    {loading && !inventory.length ? (
                        <p className="text-center py-10 text-gray-400 font-black uppercase text-xs animate-pulse">Loading Inventory...</p>
                    ) : inventory.map((item) => {
                            ...
                            {inventory.map((item) => {
                                ...
                                return (
                                    <tr key={item.id} className="hover:bg-orange-50/20 transition-colors group">
                                        <td className="p-6 font-black uppercase tracking-tight text-gray-900">{item.product_name}</td>
                                        <td className="p-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{item.sku || 'N/A'}</td>
                                        <td className="p-6 text-center">
                                            <span className={`text-xl font-black ${isLowStock ? 'text-red-500' : 'text-gray-900'}`}>{item.current_stock}</span>
                                        </td>
                                        <td className="p-6 text-center">
                                            <span className={`px-3 py-1 rounded-md text-[9px] font-black uppercase border ${isLowStock ? 'bg-red-50 text-red-600 border-red-200 animate-pulse' : 'bg-green-50 text-green-600 border-green-200'}`}>
