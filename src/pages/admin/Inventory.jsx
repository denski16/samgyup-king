                        ...
                        <table className="w-full text-left border-collapse min-w-[1000px]">
                            ...
                            <tbody className="divide-y divide-gray-100 text-xs md:text-sm">
                                {loading ? (
                                    ...
                                    <tr><td colSpan="9" className="p-10 md:p-20 text-center text-gray-400 font-bold uppercase tracking-[0.3em] text-xs">No items found</td></tr>
                                ) : (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                                <div className="sm:col-span-1">
                                    ...
                                    <input required className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 text-xs md:text-sm font-mono uppercase" value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value.toUpperCase() })} />
                                </div>
                                <div className="sm:col-span-2">
                                    <label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Product Name</label>
                                    <input required className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 text-xs md:text-sm font-black uppercase" value={formData.product_name} onChange={(e) => setFormData({ ...formData, product_name: e.target.value })} />
                                </div>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                                <div><label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Cost</label><input type="number" step="0.01" className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl text-xs md:text-sm font-bold" value={formData.cost_per_unit} onChange={(e) => setFormData({ ...formData, cost_per_unit: parseFloat(e.target.value) || 0 })} /></div>
                                <div><label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Price</label><input type="number" step="0.01" className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl text-orange-600 font-black text-xs md:text-sm" value={formData.price_per_unit} onChange={(e) => setFormData({ ...formData, price_per_unit: parseFloat(e.target.value) || 0 })} /></div>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                                <div><label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Initial</label><input type="number" className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl text-xs md:text-sm font-bold" value={formData.initial_quantity} onChange={(e) => setFormData({ ...formData, initial_quantity: parseInt(e.target.value) || 0 })} /></div>
                                <div><label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Re-order</label><input type="number" className="w-full bg-gray-50 p-3 md:p-4 rounded-xl md:rounded-2xl text-red-500 font-black text-xs md:text-sm" value={formData.re_order_level} onChange={(e) => setFormData({ ...formData, re_order_level: parseInt(e.target.value) || 0 })} /></div>
                            ...
                            <button type="submit" className="w-full p-4 md:p-5 bg-orange-600 hover:bg-orange-700 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl mt-4 transition-all active:scale-95 text-[10px]">
